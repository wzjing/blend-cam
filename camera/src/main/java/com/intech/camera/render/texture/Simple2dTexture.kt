 ...
 * 配合相芯绘制NV21格式的数据
 */
class Simple2dTexture: BaseTexture(VERTEX_SHADER, FRAGMENT_SHADER) {
...
"""
    ...
    private fun getLocations() {
        ...
        GlUtil.checkLocation(maPositionLoc, "$TAG.aPosition")
        maTextureCoordLoc = GLES20.glGetAttribLocation(mProgramHandle, "aTextureCoord")
        GlUtil.checkLocation(maTextureCoordLoc, "$TAG.aTextureCoord")
        muMVPMatrixLoc = GLES20.glGetUniformLocation(mProgramHandle, "uMVPMatrix")
        GlUtil.checkLocation(muMVPMatrixLoc, "$TAG.uMVPMatrix")
    }
    ...
    override fun drawOffScreen(
        ...
        logger.trace(TAG, "drawOffScreen :: width = $width, height = $height, rotation = $rotation, texId = $texId, mvpMat = ${mvpMatrix.contentToString()}\n")
